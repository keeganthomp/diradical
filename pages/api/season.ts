  ...
  switch (req.method) {
    ...
    case 'POST': {
      ...
      try {
        ...
        const fmtData = usersWithPlays.map((user) => {
          ...
          const playsPerTrack = user.events.reduce((acc, cur) => {
            if (acc[cur.trackId]) {
              ...
              acc[cur.trackId] = {
                total: 1,
                artistAccountId: cur.track.artist.stripeAccountId,
              }
            }
            return acc
          }, {})
          const percentPerTrack = Object.keys(playsPerTrack).map((trackId) => {
            const { total, artistAccountId } = playsPerTrack[trackId]
            return {
              trackId,
              percent: total / totalPlays,
              artistAccountId,
            }
          })
          return {
            userId: user.id,
            payouts: percentPerTrack,
          }
        })
